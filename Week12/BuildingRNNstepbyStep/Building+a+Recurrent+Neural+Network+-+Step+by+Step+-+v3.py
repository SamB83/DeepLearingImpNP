...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    #Sam: n_x would be length of the dictionary. Each column wise vector in the matrix (n_x,m) would be a
    # one hot vector denoting the word at instance <t> for that input example. 
    # The vectorization is occurring over each instances of all examples at one time since, for calc the 
    # the next a<t> we need a<t-1> for each eaxample.
    Wax = parameters["Wax"]
    Waa = parameters["Waa"]
    Wya = parameters["Wya"]
    ba = parameters["ba"]
    ...
    a_next = np.tanh(np.dot(Waa,a_prev) + np.dot(Wax,xt) + ba)#Sam : (n_a,m)+(n_a,m)+(n_a,1) = (n_a,m)
    # compute output of the current cell using the formula given above
    yt_pred = softmax(np.dot(Wya,a_next) + by) #Sam:  (n_y,m)+(n_y,1)=(n_y,m)
    ### END CODE HERE ###
...
def rnn_forward(x, a0, parameters):
    ...
    for t in range(T_x):
        ...
        # Save the value of the new "next" hidden state in a (≈1 line) ; stores prediction a<t> for instance <t> in 
        # matrix (n_a, m, T_x)
        a[:,:,t] = a_next
        # Save the value of the prediction in y (≈1 line);stores prediction y_pred<t> for instance <t> in 
        # matrix (n_y, m, T_x)
        y_pred[:,:,t] = yt_pred
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    ### START CODE HERE ###
    # Concatenate a_prev and xt (≈3 lines) ; vertically stack a_prev(n_a, m) and xt(n_x, m)
    ...
    # Compute prediction of the LSTM cell (≈1 line) ####Sam: from disc forum...plz note the formulae here for yt_pred
...
def rnn_cell_backward(da_next, cache):
    ...
    https://www.coursera.org/learn/nlp-sequence-models/discussions/weeks/1/threads/hSEk2EINEeiosQpV5z7REg?sort=createdAtDesc
    """
...
def rnn_backward(da, caches):
    ...
    <Snip> <Snip> <Snip> rnn_cell_forward
    ...
    for t in reversed(range(T_x)):
        ...
        #https://www.coursera.org/learn/nlp-sequence-models/discussions/weeks/1/threads/hSEk2EINEeiosQpV5z7REg?sort=createdAtDesc
        gradients = rnn_cell_backward(da[:,:,t] + da_prevt, caches[t]) # key note : added da_prevt 
