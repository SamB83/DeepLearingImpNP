...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.absolute((1 - (np.linalg.norm(mu_orth))**2)))*((e_w1B - mu_B)/np.absolute((e_w1 - mu_orth) - mu_B))
    corrected_e_w2B = np.sqrt(np.absolute((1 - (np.linalg.norm(mu_orth))**2)))*((e_w2B - mu_B)/np.absolute((e_w2 - mu_orth) - mu_B))
